...
class RetrievalDataset(Dataset):
    ...
    def _get_text(self, caption_dict):
        ...
        A caption tuple has the form:
            (query, s, e, captions, captions_mask)
        where:
            query        : global caption text string.
            s            : temporal start (may be None).
            e            : temporal end (may be None).
            captions     : list of sentence-level temporal caption strings.
            captions_mask: binary mask (1 for real sentences, 0 for padding).
        ...
        Returns:
            ...
            s                  : temporal start (may be None).
            e                  : temporal end (may be None).
            captions_ids       : list of np.array, sentence-level token IDs.
            captions_mask      : list, binary mask for real vs padded sentences.
            captions_word_mask : list of np.array, sentence-level attention masks.
        """
        query, s, e, captions, captions_mask = caption_dict

        # Tokenize the global aggregated caption
        query_ids, query_mask = self._processing_caption(query)

        # Tokenize each sentence-level caption independently
        captions_ids, captions_word_mask = [], []
        for caption in captions:
            caption_ids, caption_word_mask = self._processing_caption(caption)
            captions_ids.append(caption_ids)
            captions_word_mask.append(caption_word_mask)

        return query_ids, query_mask, s, e, captions_ids, captions_mask, captions_word_mask
    ...
    def _get_rawvideo(self, video_id, s=None, e=None):
        ...
        Args:
            ...
            s: Optional start frame index.
            e: Optional end frame index.
        Returns:
    ...
    def __getitem__(self, idx):
        ...
        Uses the decord backend by default for video decoding.
        Returns:
            Tuple of (text_ids, text_mask, text_ids_list, text_mask_list,
                      text_word_mask_list, video, video_mask, idx,
                      hash(video_id)).
        """
        video_id, caption_dict = self.sentences_dict[idx]

        text_ids, text_mask, s, e, text_ids_list, text_mask_list, text_word_mask_list = self._get_text(caption_dict)
        # video, video_mask = self._get_rawvideo_dec(video_id, s, e)
        video, video_mask = self._get_rawvideo(video_id, s, e)
        return text_ids, text_mask, text_ids_list, text_mask_list, text_word_mask_list, video, video_mask, idx, hash(video_id.replace("video", ""))
